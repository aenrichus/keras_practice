...
def iceberg():
    """
    Model is defined here: Currently a 4-layer convnet.
    :return: model
    """
    model = models.Sequential()

    # conv layer one has 32 feature maps, 3x3 kernel, based on 75x75 input image
    model.add(layers.Conv2D(256, (3, 3), input_shape=(75, 75, 3)))
    ...
    model.add(layers.Conv2D(768, (3, 3)))
    model.add(layers.Activation('relu'))
    model.add(layers.BatchNormalization())
    model.add(layers.Dropout(0.5))
    model.add(layers.Conv2D(768, (3, 3)))
    ...
    model.add(layers.Dense(8192))  # fully connected = Dense
    model.add(layers.Activation('relu'))
    model.add(layers.BatchNormalization())
    model.add(layers.Dropout(0.5))
    ...
    adam_opt = optimizers.Adam(lr=0.00025, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
